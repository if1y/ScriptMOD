...
# 下载主程序及相关依赖（改为使用天灵的强化修复版本）
_DOWNLOAD(){
	[[  -e "${FOLDER}" ]] && rm -rf "${FOLDER}"
	echo -e "${INFO} 开始安装 ${NAME} ..."
	env GIT_SSL_NO_VERIFY=true git clone -b enhanced https://github.com/1715173329/UnblockNeteaseMusic.git "${FOLDER}"
...
# 安装系统服务（改到魔改版仓库，简化文件名，增加后缀名）
_SERVICE(){
	if [[ "${SYSTEM_RELEASE}" = "centos" ]]; then
		wget --no-check-certificate "https://cdn.jsdelivr.net/gh/if1y/ScriptMOD/unblock163/centos.service" -O "/etc/init.d/${NAME_SERVICE}"
		...
		wget --no-check-certificate "https://cdn.jsdelivr.net/gh/if1y/ScriptMOD/unblock163/debian.service" -O "/etc/init.d/${NAME_SERVICE}"
		[[ !  -e "/etc/init.d/${NAME_SERVICE}" ]] && echo -e "${ERROR} ${NAME} 服务管理脚本下载失败 !" && _INSTALLATION_FAILURE_CLEANUP
		chmod +x "/etc/init.d/${NAME_SERVICE}"
...
_SET(){
 ...
 ${GREEN_FONT_PREFIX}2.${FONT_COLOR_SUFFIX}  修改 音源使用
...
_SOURCE_SET() {
	...
	echo && echo "------------------------"
	echo -e "	音源使用 : ${RED_BACKGROUND_PREFIX} ${SOURCE} ${FONT_COLOR_SUFFIX}"
	echo "------------------------" && echo
}
# 修改 严格模式
_STRICT_SET() {
...
_VIEW(){
	...
	音源使用: ${GREEN_FONT_PREFIX}${SOURCE}${FONT_COLOR_SUFFIX}
...
# 更新脚本（改到魔改版仓库）
_UPDATE_SHELL(){
	NEW_VER_SHELL=$(wget --no-check-certificate -qO- -t1 -T3 "https://cdn.jsdelivr.net/gh/if1y/ScriptMOD/unblock163/unblock163.sh"|grep 'NOW_VER_SHELL="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z "${NEW_VER_SHELL}" ]] && echo -e "${ERROR} 获取脚本最新版本失败！无法链接到 Github !" && exit 1
		...
		wget -N --no-check-certificate "https://cdn.jsdelivr.net/gh/if1y/ScriptMOD/unblock163/unblock163.sh"
